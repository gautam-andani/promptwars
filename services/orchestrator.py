"""Pipeline orchestration for Side Quest.
...
async def _generate_json(client: LLMClient, prompt: str) -> dict[str, Any]:
    ...
    try:
        ...
        logger.warning(
            "%s returned invalid JSON (len=%d, tail=%r); retrying once with a "
            "stricter instruction",
            client.name, len(raw), raw[-120:],
        )
    ...
    try:
        async with httpx.AsyncClient(
            ...
            response = await client.get(
                url,
                headers={"User-Agent": "SideQuest/1.0 (travel discovery app)"},
            )
...
async def _hydrate_photos(attractions: list[Attraction]) -> None:
    ...
    Falls back to the place name as the lookup title when the LLM did not
    supply a wikipedia_title — Wikipedia's summary endpoint resolves
    redirects, so plain place names often still land on the right article.
    """
    targets = [a for a in attractions if not a.photo_url]
    if not targets:
        return
    thumbnails = await asyncio.gather(
        *(_fetch_wikipedia_thumbnail(a.wikipedia_title or a.name) for a in targets)
    )
    hydrated = 0
